...
void dispatcher::loop()
{
	while (true)
	{
		op __operand = *((op*)_opptr);
		_opptr += (uint64_t)sizeof(op);
		...
		case op::l_alloc:
		{
			uint64_t size = (uint64_t) * ((uint16_t*)_opptr);
			_opptr += (ptrdiff_t)sizeof(uint16_t);
		...
		case op::s_alloc:
		{
			uint64_t size = (uint64_t) * ((uint8_t*)_opptr);
			_opptr += (ptrdiff_t)sizeof(uint8_t);
